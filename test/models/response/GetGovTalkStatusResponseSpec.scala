...
class GetGovTalkStatusResponseSpec extends AnyWordSpec with Matchers {
  ...
  "GetGovTalkStatusResponse.format" should {
    ...
    "serialize and deserialize correctly" in {
      ...
      val model = GetGovTalkStatusResponse(
        govtalk_status = Seq(record)
      )

      val json = Json.toJson(model)

      json.validate[GetGovTalkStatusResponse] mustBe JsSuccess(model)
    }
  }
}
