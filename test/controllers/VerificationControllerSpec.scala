...
class VerificationControllerSpec extends SpecBase with EitherValues {
  ...
  "getSubmittedVerifications" - {
    ...
    val validRequest = GetSubmittedVerificationsRequest(
      instanceId = "abc-123"
    )
    ...
    "returns 200 OK with JSON body when service succeeds" in {
      ...
      when(verificationService.getSubmittedVerifications(eqTo(validRequest))(any[HeaderCarrier]))
        .thenReturn(Future.successful(response))
      ...
      val result = controller.getSubmittedVerifications()(req)
  ...
  "proceedUnmatchedVerification" - {
    ...
    "returns 200 when service succeeds" in {
      ...
      when(verificationService.proceedUnmatchedVerification(eqTo(validRequest))(any[HeaderCarrier]))
        .thenReturn(Future.successful(()))
    ...
    "returns 502 BadGateway with error body when service fails" in {
      ...
      status(result) mustBe BAD_GATEWAY
      contentType(result) mustBe Some(JSON)
      contentAsJson(result) mustBe Json.obj("message" -> "proceed-unmatched-verification-failed")
